...
if st.session_state.page == 'welcome':
    ...
    if col1.button("START QUIZ"):
        if name:
            ...
            st.session_state.update({'page': 'quiz', 'p_name': name, 'time_limit': limit, 'start_time': time.time(), 'score': 0, 'shuffled_indices': shuffled_indices, 'current_step': 0, 'wrong_answers': []})
            st.rerun()
    ...
    if step < len(st.session_state.shuffled_indices):
        ...
        st.markdown(f"""<div class="question-box"><p style="opacity:0.6; font-size:14px; margin:0;">QUESTION {step+1}</p><h2 style="margin-top:10px;">{q['question']}</h2></div>""", unsafe_allow_html=True)
